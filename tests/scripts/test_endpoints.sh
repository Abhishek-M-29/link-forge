...
REG_RESPONSE=$(curl -s -X POST http://127.0.0.1:8000/api/v1/auth/register \
  ...
  -d '{"username": "testuser_sh", "email": "test_sh@example.com", "password": "password123"}')
echo $REG_RESPONSE
...
LOGIN_RESPONSE=$(curl -s -X POST http://127.0.0.1:8000/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email": "test_sh@example.com", "password": "password123"}')
echo $LOGIN_RESPONSE
...
ANALYTICS_RESPONSE=$(curl -s -X GET http://127.0.0.1:8000/api/v1/urls/$URL_ID/analytics \
  -H "Authorization: Bearer $ACCESS_TOKEN")
echo $ANALYTICS_RESPONSE

echo -e "\n${GREEN}=== 6. List URLs (Pagination) ===${NC}"
LIST_RESPONSE=$(curl -s -X GET "http://127.0.0.1:8000/api/v1/urls?page=1&page_size=10" \
  -H "Authorization: Bearer $ACCESS_TOKEN")
echo $LIST_RESPONSE

echo -e "\n${GREEN}=== 7. Deactivate URL ===${NC}"
PATCH_RESPONSE=$(curl -s -X PATCH http://127.0.0.1:8000/api/v1/urls/$URL_ID \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"is_active": false}')
echo $PATCH_RESPONSE
...
echo -e "\n${GREEN}All endpoints tested successfully!${NC}"
